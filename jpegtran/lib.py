...
class Exif(object):
    def __init__(self, blob):
        ...
        if alignstr == b'II':
            ...
            raise InvalidExifData("Invalid byte alignment: {0}".format(alignstr))
    ...
    @property
    def thumbnail(self):
        offset = (self._exif_start + self._unpack('I', self._get_tag_offset(0x201) + 8))
    ...
    def thumbnail(self, data):
        ...
        try:
            ...
            stripped_data = data
        self._pack('>H', app1_size_offset, app1_size + (len(stripped_data) - old_size))
    ...
    def _get_tag_offset(self, tagnum):
        ...
        while True:
            ...
            if p_ifd == 0:  # This means there are no more IFDs
                raise ExifTagNotFound("Could not find EXIF Tag {0}".format(tagnum))
...
def jpegtran_op(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        rv = lib.tjTransform(tjhandle, in_data_p, in_data_len, 1, out_bufs, out_sizes, transformoption, 0)
        if rv < 0:
...
class Transformation(object):
    ...
    def get_dimensions(self):
        ...
        inbuf = ffi.buffer(in_data_p, in_data_len)
        inbuf[:] = self._data
        img = ffi.gc(lib.epeg_memory_open(in_data_p, in_data_len), lib.epeg_close)
    ...
    def flip(self, direction):
        ...
        if direction == 'vertical':
            ...
            raise ValueError("Invalid direction, must be 'vertical' or " "'horizontal'")
        return options
