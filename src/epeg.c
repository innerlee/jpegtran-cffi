...
EAPI void epeg_decode_bounds_set(Epeg_Image *im, int x, int y, int w, int h) {
    if (im->pixels)
        return;
    if (w < 1)
        w = 1;
 ...
 */
EAPI void epeg_pixels_free(Epeg_Image *im, const void *data) { free((void *)data); }
 ...
 *
 */
EAPI const char *epeg_comment_get(Epeg_Image *im) { return im->in.comment; }
 ...
 *
 */
EAPI void epeg_thumbnail_comments_enable(Epeg_Image *im, int onoff) { im->out.thumbnail_info = onoff; }
...
static int _epeg_decode(Epeg_Image *im) {
    ...
    while (im->in.jinfo.output_scanline < im->in.jinfo.output_height) {
        ...
        jpeg_read_scanlines(&(im->in.jinfo), &(im->lines[im->in.jinfo.output_scanline]),
...
static int _epeg_scale(Epeg_Image *im) {
    ...
    for (y = 0; y < h; y++) {
        row = im->pixels +
              (((y * im->in.jinfo.output_height) / h) * im->in.jinfo.output_components * im->in.jinfo.output_width);
...
static int _epeg_trim(Epeg_Image *im) {
    ...
    for (y = 0; y < h; y++)
        im->lines[y] = im->pixels + ((y + b) * im->in.jinfo.output_components * im->in.jinfo.output_width) +
